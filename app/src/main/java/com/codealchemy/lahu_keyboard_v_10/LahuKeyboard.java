...
/**
 * Created by Khant Naing Set on 1/26/2017.
 */

public class LahuKeyboard extends Keyboard {
    ...
    public LahuKeyboard(Context context, int layoutTemplateResId, CharSequence characters, int columns, int horizontalPadding) {
        super(context, layoutTemplateResId, characters, columns, horizontalPadding);
    }

    public LahuKeyboard(Context context, int xmlLayoutResId) {
        super(context, xmlLayoutResId);
    }
    @Override
    protected Key createKeyFromXml(Resources res, Row parent, int x, int y,
                                   XmlResourceParser parser) {
        Key key = new LahuKey(res, parent, x, y, parser);
        if (key.codes[0] == 10) {
            mEnterKey = key;
        } else if (key.codes[0] == ' ') {
            mSpaceKey = key;
            ...
            mModeChangeKey = key;
            mSavedModeChangeKey = new LahuKey(res, parent, x, y, parser);
        } else if (key.codes[0] == LahuKeyboardView.KEYCODE_LANGUAGE_SWITCH) {
            mLanguageSwitchKey = key;
            mSavedLanguageSwitchKey = new LahuKey(res, parent, x, y, parser);
        }
        return key;
    }
    ...
    static class LahuKey extends Keyboard.Key {

        public LahuKey(Resources res, Keyboard.Row parent, int x, int y,
                        XmlResourceParser parser) {
